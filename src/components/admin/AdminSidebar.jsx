...
const AdminSidebar = () => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div 
        className={`${isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'} ${isOpen && !isMobile ? 'w-64' : 'w-20'} 
      ...
      >
        ...
        <div className="mt-6 px-4">
          <div className={`${(isOpen && !isMobile) || isMobile ? 'block' : 'hidden'} mb-6 px-4`}>
            <h2 className="text-xs tracking-wider uppercase text-gray-400 font-semibold">Main Menu</h2>
          </div>
        </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.name}
                to={item.path}
                className={`flex items-center justify-${(isOpen && !isMobile) || isMobile ? 'start' : 'center'} px-4 py-3 rounded-lg mb-2 transition-all duration-200 group
                  ${isActive 
                    ? 'bg-primary-50 text-primary-600 shadow-sm' 
                    : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'}`}
              >
                <div className={`flex-shrink-0 ${isActive ? 'text-primary-600' : 'text-gray-500 group-hover:text-gray-700'}`}>
                  {item.icon}
                </div>
        ...
        <div className="absolute bottom-0 w-full border-t border-gray-200 p-2">
          <div className={`px-2 py-2 rounded-lg hover:bg-gray-100 ${isOpen ? 'block' : 'hidden'} md:hidden`}>
          ...
          >
            ...
            {((isOpen && !isMobile) || isMobile) && <span className="ml-3 text-sm">Logout</span>}
          </button>
        </div>
      </div>
    </>
